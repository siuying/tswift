...
- agent verify-review-fs: PASS — own repros green (write/read/list/copy/move/delete round trip, nested createDirectory, attributesOfItem size/isDir, documents-dir persistence within run, do-catch on missing file catches CocoaError code 4, UserDefaults Data/dict/register(defaults:) round trip, String write(to:)/contentsOf via same sandbox); sandbox holds (../ and /etc/passwd escapes fail safely, no leak/write outside root; symlink components rejected; TSWIFT_FS_ROOT honored); scope.toml documents symlinks/permissions/FileHandle/Date-defaults unsupported; presubmit green. Verdict: With fixes (minor — sandbox-rejection error codes inconsistent across read/exists/write vs list/remove; TOCTOU symlink check).

## agent geometry-navigation

- Deepened everyday SwiftUI geometry/navigation/controls. **GeometryReader** (0%→real): prelude `CGSize`/`CGPoint`/`CGRect`/`CoordinateSpace`/`GeometryProxy`; the `(GeometryProxy)->Content` closure runs against a *deterministic* headless proxy (`size` = runtime default, `safeAreaInsets` zero, `frame(in:)` at origin) — realized children serialize with `proposedWidth/Height` args. Tiers named honestly: headless-default (ships) vs host-measured (future, geometry-event tripwire); no device-pixel parity claimed. **NavigationSplitView** (0%→real): 2/3-column eager render (sidebar first, detail last) + `columns` arg; selection-driven detail is host-driven (honest scope). **ScrollView** `showsIndicators`/`axes` now real args; **RoundedRectangle** gains `cornerSize:`/`style:` (typed `progressViewStyle` to `_ControlStyle` to dodge the `.circular` collision). **ForEach** keyed insert/remove minimal patches verified.
- **Blocker:** `ForEach($items)` collection-binding form — per-element `Binding` synthesis needs interpreter-owned get/set closures over the parent binding+index, which the Rust view-init seam cannot fabricate; fails with an explicit diagnostic. Value/programmatic nav (NavigationLink(value:), navigationDestination(for:), NavigationPath append/removeLast push/pop) already covered by `navigation-values.*`.
- Fixtures: `geometry-reader`, `navigation-split`, `shapes-scroll` (UIIR goldens), `foreach-mutation` (insert/remove patch golden). Coverage SwiftUI verified 467/692 (67.5%) → 475/692 (68.6%); implemented 485/692 (70.1%) → 498/692 (72.0%). GeometryReader 0%→100% impl/50% verif, GeometryProxy 0%→50%, NavigationSplitView 0%→100%, ScrollView 20%→60% impl, RoundedRectangle 12.5%→37.5% impl, ForEach 25%→75% impl. Commits below. No wasm/web source touched (new node kinds are host-applier additions per ADR-0006).

- agent verify-review-geo: PASS — independently reproduced GeometryReader proxy (320×480, frame.midX=160, safe=0), nested reader, NavSplitView 2/3-col, ScrollView(.horizontal, showsIndicators:false), RoundedRectangle cornerSize/style, keyed ForEach insert(id=9@1)/removeLast(id=3) minimal patches + reverse→move ops (not remove+insert), ForEach($items) explicit diagnostic, navigation-values push/pop golden intact. Coverage baseline recomputed at 7766b9b = 467 → 475 verified (+8 exact). presubmit green. Verdict: With fixes (non-blocking) — verified/init/body credited by generic tokens is a preexisting methodology weakness, not new inflation; several new args (cornerSize/style/axes/showsIndicators) conservatively undercounted as implemented-only. Approve.

- agent website-sync-2: Synced website to runtime @5bcb6ee. Regenerated `website/src/data/coverage/*.json` from live registry dumps (std/foundation/swiftui/charts/eventkit/swiftdata `dump_registered_keys`) — SwiftUI totals 467→475 impl, 18→23 partial (72.0% impl / 68.6% verif); stdlib 468/578 (81.0%), Foundation 467/615 (75.9%) unchanged in registry. Updated SwiftUI tier roadmap (Tier 6 async lifecycle ⬜→🟡, Tier 7 GeometryReader ⬜→🟡, Tier 8 NavigationSplitView) + prose; added Foundation "Host services" section (sandboxed FileManager/UserDefaults, CocoaError, route-table transport). Rebuilt WASM (`npm run build`), `scripts/validate web` green (17 pages, all studio/wasm checks pass).

## agent nits-cleanup

Cleared accumulated minor review nits across the fs/String/Sequence/coverage slices; presubmit green.

- **fs sandbox codes + TOCTOU** (verify-review-fs): every host fs op now surfaces a sandbox escape as the same CocoaError permission code (513) — was a per-op mix of not-found 4 (read→null→Data(contentsOf:)), generic 512 (list/remove/copy/move), and silent `false` (write). read/write now throw on escape; `FileManager.contents(atPath:)` stays non-throwing by mapping a thrown read to nil. `resolve()` canonicalizes the deepest existing ancestor and re-checks sandbox containment, shrinking the check-then-open symlink-swap window to the not-yet-existing leaf only (std has no openat/O_NOFOLLOW). New `sandbox_escape_uses_one_permission_code_across_operations` test.
- **String.IndexRange dedup** (review-phase2bc): the private `_lowerOffset`/`_upperOffset` decode was copied in storage.rs, stdlib.rs, and substring.rs — folded into one exported `tswift_core::index_range_offsets`; behavior unchanged.
- **range(of: "")** (review-phase2bc): now returns nil (Foundation semantics) instead of an empty range; `range_of` also segments the receiver window once via new `window_graphemes` (str_of+graphemes re-split a Substring twice). New `range_of_empty_needle_returns_nil` test.
- **Negative prefix/drop counts** (verify-review-sequence): `prefix(-1)`/`suffix(-1)`/`dropFirst(-1)`/`dropLast(-1)` now trap with Swift's precondition message instead of silently clamping to the default; matches the already-trapping IndexPath path. New `negative_prefix_and_drop_counts_trap` test.
- **Coverage methodology** (verify-review-geo): documented the token-based-verification limitation (fixture tokens aren't type-scoped, so generic member names like init/body/map credit any registered type once any type exercises the name) in the framework-inventory README + a comment at the matching site. `implemented` stays registry-gated/exact.

Skipped / deferred:
- Grapheme recompute: fixed the clear `range_of` case (via `window_graphemes`); did **not** sweep every `graphemes(&str_of(..))` site — a broad str_of restructure is invasive for marginal gain (tripwire).
