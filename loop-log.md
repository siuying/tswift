...
| 17 | 1c294b6 | Key paths `\Root.path` + key-path functions | keep | lexer `\` sigil; KeyPathExpr node; key path = Closure(ClosureDef::KeyPath) so map/filter work free; `[keyPath:]` read/write incl nested & class-in-place; `\.self` identity. Reviewed by gpt-5.5: fixed identity-replacement writeback (ptr_eq) + arity strictness; accepted permissive `[keyPath:]` label (only `\` produces keypath closures) |
