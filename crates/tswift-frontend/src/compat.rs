...
//!
//! This module is intentionally internal: `tswift-core` keeps consuming
...
impl RuntimeAst {
    pub(crate) fn analyze(source: &str) -> RuntimeAst {
        let mut ast = match tswift_parser::parse(source) {
            Ok(ast) => ast,
    ...
    fn lower_node(&mut self, node: tswift_ast::Node<'_>) -> NodeId {
        ...
        let text = match node.kind() {
            tswift_ast::NodeKind::CompilerDirective => {
        ...
        if node.kind() == tswift_ast::NodeKind::CastExpr && node.text() == Some("as?") {
        ...
        if node.kind() == tswift_ast::NodeKind::RepeatStmt {
    ...
    fn lower_conditional(&mut self, node: tswift_ast::Node<'_>) -> NodeId {
        ...
        if node.kind() == tswift_ast::NodeKind::WhileStmt {
            self.nodes[id.0].loop_label = node.text().map(ToOwned::to_owned);
        }
        let mut children: Vec<NodeId> = Vec::new();
        for child in node.children() {
    ...
    fn lower_optional_binding(&mut self, node: tswift_ast::Node<'_>) -> NodeId {
        use tswift_ast::NodeKind as K;
        let mut name: Option<String> = None;
        let mut binding_pattern: Option<tswift_ast::Node<'_>> = None;
