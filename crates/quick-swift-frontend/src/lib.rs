//! The quick-swift frontend: a pure-Rust Swift frontend.
//!
//! This crate is the **only** runtime-facing seam onto the Swift frontend. It
//! drives the pure-Rust pipeline (`swift-lexer` → `swift-parser` → `swift-sema`)
//! and lowers the result, through the compatibility lowerer in [`compat`], into
//! the stable runtime-facing AST contract the runtime (`quick-swift-core` /
