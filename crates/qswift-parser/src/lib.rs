...
//!
//! [`parse`] turns a [`qswift_lexer`] token stream into a [`qswift_ast::Ast`].
...
use qswift_ast::{Ast, NodeId, NodeKind};
use qswift_lexer::{tokenize, Token, TokenKind};
...
mod tests {
    ...
    fn first_stmt(ast: &Ast) -> qswift_ast::Node<'_> {
    ...
    fn computed_property_and_observers_parse_accessors() {
        ...
        let binding_name = |v: &qswift_ast::Node<'_>| -> Option<String> {
            v.children()
