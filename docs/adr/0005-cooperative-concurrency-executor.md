...
recursive tree-walker (`crates/quick-swift-core/src/interp.rs`) can pause at an
...
- `unsafe` confinement (ADR-0001) is preserved: stack switching lives in
  ...
  boundary, encapsulated in `crates/quick-swift-core/src/concurrency.rs`.
- Acceptance fixtures live under `crates/quick-swift-cli/tests/fixtures/` and
