...
The #36 experiment showed that replacing the `quick-swift-frontend` backend with
`swift-parser` + `swift-sema` is not blocked mainly by parse acceptance. It is blocked
...
forced every mismatch to leak into `quick-swift-core`, producing a broad set of failing
runtime tests.

## 2. Decision

Do **not** keep patching the runtime for Rust AST differences. Instead, introduce one
deep compatibility module inside `quick-swift-frontend`:

```text
swift-lexer/parser/sema clean AST
          │
          ▼
quick-swift-frontend::compat lowerer
          │
          ▼
...
Analysis / Node / NodeKind public facade
          │
          ▼
quick-swift-core / quick-swift-std unchanged
```

The seam remains `quick-swift-frontend::{Analysis, Node, NodeKind}`. The new compat
lowerer hides the messy shape conversion behind that seam.

## 3. Non-goals

- Do not rewrite `quick-swift-core` to understand both msf and Rust AST shapes.
- Do not delete `msf-sys` until the Rust backend passes the full workspace test suite.
- Do not make `swift_ast::NodeKind` a clone of msf. Keep `swift_ast` clean and lower
...
Add an internal module, e.g. `crates/quick-swift-frontend/src/compat/`, containing:
...
- `swift_ast::NodeKind`: clean parser/sema IR.
- `quick_swift_frontend::NodeKind`: stable runtime-facing compatibility vocabulary.
...
Create `crates/quick-swift-frontend/src/compat/contract.rs` or a markdown spec listing,
...
- known runtime consumers in `quick-swift-core`
...
1. **Compat unit tests** in `quick-swift-frontend`:
   - parse Rust AST
   - run lowerer
...
3. **Runtime golden tests**:
   - no fixture is considered fixed until the corresponding `quick-swift-core` or
     `quick-swift-cli` runtime test passes on the Rust backend
...
- Keep `quick-swift-frontend::{Analysis, Node, NodeKind}` as the only runtime-facing
  interface.
...
- Add a manifest of runtime-facing AST snapshots under
  `crates/quick-swift-cli/tests/fixtures/ast/` or a new
  `crates/quick-swift-frontend/tests/fixtures/compat/` directory.
...
- Define our own modifier bit layout in `quick-swift-frontend`; stop depending on C enum
  values.
...
- Preserve child order used by `quick-swift-core::match_pattern`.
...
- `quick-swift-core` has no Rust-vs-msf AST special cases.
- `quick-swift-frontend` owns the runtime-facing `NodeKind` contract.
