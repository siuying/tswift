...
| [x] | Capture lists `[weak self]` `[unowned]` (weak zeroes after dealloc — the captured chain does not retain the referent, incl. alias captures `[weak o = self]` and names beside mutable siblings (per-binding env cells); unowned traps on post-dealloc reads; `guard let self = self` + `if let self` shorthand rebinding; value captures `[y = expr]`. Known limit: closures capture whole environments, so a *different* strong binding in scope — e.g. the implicit method `self` when only a local alias is weak-captured, or a sibling closure's chain — can still retain the object; exact capture analysis is future work) | ✅ | ★★★ | R3 |
